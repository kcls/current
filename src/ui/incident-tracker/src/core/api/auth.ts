...
export interface TokenClaims {
  ...
  expires_at: number;
  session_id: string;
  /** Working org unit's stable uuid (single claim since the 1d.3 flip). */
  org_unit?: string;
  sub_uuid?: string;
  display_name?: string;
}
...
function decodeAndCacheToken(token: string): void {
  try {
    ...
    cachedSessionData = {
      // org_unit carries the uuid since the 1d.3 flip; org_unit_uuid is
      // the pre-flip dual claim (tolerated for a still-cached old token).
      org_unit:
        typeof payload.org_unit === 'string' ? payload.org_unit : payload.org_unit_uuid,
      user_id: payload.sub ? parseInt(payload.sub, 10) : undefined,
      user_uuid: payload.sub_uuid,
      email: payload.email,
...
export const authApi = {
  ...
  /** The working org unit's uuid, as carried in the access token. */
  getOrgUnit(): string | undefined {
  ...
  /**
   * Check whether the current user holds a permission. org_unit_uuid
   * omitted = checked at the root org unit (matches server-side
   * permission_required).
   */
  async userHasPerm(perm: string, orgUnitUuid?: string): Promise<boolean> {
    try {
      const result = await authPost<{ has_perm?: boolean }>(
        '/authz/user-has-perm',
        { perm, org_unit_uuid: orgUnitUuid ?? null }
      );
  ...
  async getUser(params?: { id?: number; uuid?: string; options?: { with_working_locations?: boolean } }): Promise<any> {
